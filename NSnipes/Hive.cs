...
public class Hive(int x, int y, int snipesPerHive, DateTime? initialSpawnTime = null)
{
    ...
    public int SnipesType2 { get; set; } = snipesPerHive / 2 + (snipesPerHive % 2 == 1 ? 1 : 0); // chr(2) snipes (split evenly, extra to type A if odd)
    public int SnipesType3 { get; set; } = snipesPerHive / 2; // chr(3) snipes
